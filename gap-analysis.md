...
| Symbol | Category | Source | Platforms | Reference | Existing RustyMill impl | Breaking? | Est. size | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `raindrop` (template A: REST+token, real delta endpoint) | fn | spec | both | `connectors/raindrop.py` | — | no | M | Done (#85, subprocess binary #161) — new `dbs-connector-raindrop` crate; full incremental/reconcile/full fetch + trash polling against fixture HTTP. Since #161, this is a real `dbs-connector-raindrop` executable speaking ADR-0001's full protocol (handshake + run/stream), the first of the 14 to be one — proven end to end by a real subprocess spawn against a mock HTTP server. Still not reachable from a real `dbs backup` run: `dbs-cli`'s registry stays empty until #160 (candidate discovery) lands. `archive_permanent_copy` (opt-in, Pro-only) not ported |
