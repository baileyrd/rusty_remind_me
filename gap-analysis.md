...
| `/api` export routes | fn | spec | both | `src/dbs/web/app.py` (API routes) | — | no | S | Done (#176) — `dbs-web::api`; `GET /api/export` (a real file download via `BackupService::export` to a temp file, `Content-Type`/`Content-Disposition` from `Exporter::media_type`/`file_ext`), `POST /api/export-notes` (bridges `dbs_core::export_notes` directly) |
