...
| `mastodon` (template A) | fn | spec | both | `connectors/mastodon.py` | — | no | M | Done (#89) — new `dbs-connector-mastodon` crate; full enumeration every run (no usable delta filter), `Link`-header pagination, combined `ReconcileMarker` withheld if either kind disabled. Real `dbs-connector-mastodon` binary since #164 — but `instance` has no usable default and there's no per-source config passthrough yet, so a real `dbs backup` run needs `DBS_MASTODON_TEST_BASE_URL`-style wiring or that still-open plumbing gap closed before it can do real work |
| `bluesky` (template A) | fn | spec | both | `connectors/bluesky.py` | — | no | M | Done (#90) — new `dbs-connector-bluesky` crate; app-password session exchange, `listRecords` cursor pagination, full enumeration every run (no usable delta filter) followed by a `ReconcileMarker`. Real `dbs-connector-bluesky` binary since #164, reachable from a real `dbs backup` run — `identifier` has no per-source config passthrough yet either, but (unlike `mastodon`'s `instance`) nothing validates it's non-empty before the HTTP layer, so this one works today regardless |
...
| `podcast` (generic RSS-shaped) | fn | spec | both | `connectors/podcast.py` | — | no | M | Done (#93) — new `dbs-connector-podcast` crate; RSS 2.0/Atom parsing via `roxmltree` (first XML dep in the workspace), OPML merge+dedup, optional enclosure download, deletion detection deliberately disabled (rolling-window feeds, no `ReconcileMarker` ever). Real `dbs-connector-podcast` binary since #164 — but `feeds` is per-source business data with no config-passthrough plumbing yet (this connector has no fixed API host at all, feed URLs are its only "target"), so a real `dbs backup` run has nothing configured to fetch until that gap closes |
