...
| SQLite storage — upsert/classify/revisions | fn | spec | both | `src/dbs/storage/sqlite.py` | rusty_db (unverified) | no | M | |
| SQLite storage — browse/query + FTS5 search | fn | spec | both | `src/dbs/storage/sqlite.py` | rusty_db (unverified) | no | M | |
| SQLite storage — metrics aggregation | fn | spec | both | `src/dbs/storage/sqlite.py` | — | no | S | |
| SQLite storage — maintenance (VACUUM, WAL checkpoint, `PRAGMA optimize`, snapshot) | fn | spec | both | `src/dbs/storage/sqlite.py` | — | no | M | |
...
| Symbol | Category | Source | Platforms | Reference | Existing RustyMill impl | Breaking? | Est. size | Notes |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Export base + filter query | fn | spec | both | `src/dbs/export/base.py` | — | no | S | |
| JSON exporter | fn | spec | both | `src/dbs/export/json.py` | rusty_json (unverified) | no | S | |
| NDJSON exporter | fn | spec | both | `src/dbs/export/ndjson.py` | rusty_json (unverified) | no | S | |
