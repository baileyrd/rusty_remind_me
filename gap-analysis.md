...
| SQLite storage — FTS5 full-text index (`_ensure_fts`: virtual table + triggers + backfill, `browse_items`' MATCH-then-LIKE fallback) | fn | spec | both | `src/dbs/storage/sqlite.py` | — | no | S | **Split out of the row above while implementing #36's export/browse section** — `browse_items` there always uses the `LIKE` path; FTS5 needs its own migration-adjacent `migrate()` hook (`_ensure_fts`) this port doesn't have yet |
