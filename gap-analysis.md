...
| `skool` (template B + native HLS video download via yt-dlp/ffmpeg) | fn | spec | both | `connectors/skool.py` | — | no | L | Scaffolded (#97) — new `dbs-connector-skool` crate; config/capabilities/`auth_capture` ported, and every genuinely pure function (`__NEXT_DATA__` BFS search, lesson-field decoding, course-selector matching, Mux HLS URL reconstruction, membership/course/lesson parsing, community/course/lesson → `BackupItem` mapping) implemented and tested. `fetch()`'s acquisition (per-lesson browser page visits + Mux video sniffing — decision 3 below) is honestly blocked on issue #99, same as `reddit` (#96); TipTap rich-text→Markdown rendering is deferred to its own issue #100; resource/video download, the `.meta.json` resume pipeline, and GitHub-zip archiving are unported pipeline mechanics with no reachable caller until #99 exists |
