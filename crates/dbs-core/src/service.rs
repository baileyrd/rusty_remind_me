...
//! service call — once before a standalone `backup_source` (gated by
//! `BackupSourceOptions::reap`), or once up front in `backup_all`,
//! never once per source touched within that batch. The reference's
//! docstring is explicit about why: "a per-source reap inside a
//! parallel batch would flip a sibling's genuinely-running row" —
//! `backup --all --parallel N` has concurrent workers, and a mid-batch
//! reap could incorrectly interrupt a source whose run legitimately
//! started after the batch began but before that particular reap
//! call. `backup_all` reaps once itself, then calls `backup_source`
//! per source with `reap: false`, so the guarantee falls out of the
//! call graph rather than needing a shared "already reaped" flag.
//!
...
mod tests {
    ...
    use crate::storage::{BatchResult, ItemRow, PreparedItem};
    ...
    /// `backup_all` reaps exactly once per call, up front — never once
    /// per source touched within the batch (a per-source reap inside a
    /// batch could flip a sibling's genuinely-running row). See the
    /// module doc-comment.
    #[test]
    fn backup_all_reaps_exactly_once_across_multiple_sources() {
        let mut sources = HashMap::new();
        sources.insert("a".to_string(), test_source_config("a", "raindrop"));
        sources.insert("b".to_string(), test_source_config("b", "raindrop"));
        sources.insert("c".to_string(), test_source_config("c", "raindrop"));
