...
fn a_stub_subcommand_reports_not_yet_implemented() {
    ...
    // `capture` (#76), `research` (#77), and `version` (#78) are no
    // longer pure stubs — `verify` still is. There's no remaining
    // nested-subcommand enum (`sources`/`connectors`/`research`) that's
    // still a pure stub, so there's nothing left to cover with a
    // "nested stub" variant of this test.
